...
namespace SharpHmiAndroid
{
	public class ConsoleFragment : Fragment
	{
		...
		private string ButtonsNotificationOnButtonPress = "ButtonsNotificationOnButtonPress";
		...
		private string VRResponseAddCommand = "VRResponseAddCommand";
		private string VRResponseChangeRegistration = "VRResponseChangeRegistration";
		private string VRResponseDeleteCommand = "VRResponseDeleteCommand";
		private string VRResponseGetCapabilities = "VRResponseGetCapabilities";
		private string VRResponseGetLanguage = "VRResponseGetLanguage";
		private string VRResponseGetSupportedLanguages = "VRResponseGetSupportedLanguages";
		private string VRResponseIsReady = "VRResponseIsReady";
		private string VRResponsePerformInteraction = "VRResponsePerformInteraction";

		// VR Outgoing Notifications
		private string VRNotificationOnCommand = "VRNotificationOnCommand";
		private string VRNotificationOnLanguageChange = "VRNotificationOnLanguageChange";
		private string VRNotificationStarted = "VRNotificationStarted";
		private string VRNotificationStopped = "VRNotificationStopped";
		...
		public override void OnCreateOptionsMenu(IMenu menu, MenuInflater inflater)
		{
			...
			inflater.Inflate(Resource.Menu.console_menu, menu);
		}
		...
		public override bool OnOptionsItemSelected(IMenuItem item)
		{
			switch (item.ItemId)
			{
				...
				case Resource.Id.console_menu_rpc_list:
					showRPCListDialog();
					return true;
		...
		{
			String[] rpcListStringArary = { BCResponseAllowDeviceToConnect, BCResponseDialNumber,
				...
				VIResponseSubscribeVehicleData, VIResponseUnsubscribeVehicleData, VRResponseAddCommand,
		...
		private void CreateVRNotificationStopped()
		{
			...
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.ui_with_only_send_request, null);
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.get_language, null);
			rpcAlertDialog.SetView(rpcView);
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.on_command_notification, null);
			rpcAlertDialog.SetView(rpcView);
			rpcAlertDialog.SetTitle("OnCommand");
			...
			rpcAlertDialog.SetNegativeButton("Tx Later", (senderAlert, args) =>
			 {
				 //Method not present in BuildRPC.cs
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.vr_perform_interaction_response, null);
			rpcAlertDialog.SetView(rpcView);
			rpcAlertDialog.SetTitle("PerformInteraction");

			TextView textViewChoiceID = (TextView)rpcView.FindViewById(Resource.Id.vr_choice_id_tv);
			EditText editTextChoiceID = (EditText)rpcView.FindViewById(Resource.Id.vr_choice_id_et);

			TextView textViewResultCode = (TextView)rpcView.FindViewById(Resource.Id.vr_result_code_tv);
			Spinner spnResultCode = (Spinner)rpcView.FindViewById(Resource.Id.vr_perform_interaction_result_code_spn);
			...
			rpcAlertDialog.SetNegativeButton("Tx Later", (senderAlert, args) =>
			 {
				 ...
				 if (editTextChoiceID.Text.Equals(""))
					 AppInstanceManager.Instance.sendRpc(BuildRpc.buildVrPerformInteractionResponse(BuildRpc.getNextId(), 0, (HmiApiLib.Common.Enums.Result)spnResultCode.SelectedItemPosition));
				 else
					 AppInstanceManager.Instance.sendRpc(BuildRpc.buildVrPerformInteractionResponse(BuildRpc.getNextId(), Java.Lang.Integer.ParseInt(editTextChoiceID.Text), (HmiApiLib.Common.Enums.Result)spnResultCode.SelectedItemPosition));
		...
		private void CreateVRResponseIsReady()
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			...
			rpcAlertDialog.SetNegativeButton("Tx Later", (senderAlert, args) =>
			{
				AppInstanceManager.Instance.sendRpc(BuildRpc.buildIsReadyResponse(BuildRpc.getNextId(), HmiApiLib.Types.InterfaceType.VR, checkBoxAllow.Checked, (HmiApiLib.Common.Enums.Result)spnResultCode.SelectedItemPosition));
			});
		...
		private void CreateVRResponseGetLanguage()
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.get_language, null);
			rpcAlertDialog.SetView(rpcView);
			...
			rpcAlertDialog.SetNegativeButton("Tx Later", (senderAlert, args) =>
			{
				AppInstanceManager.Instance.sendRpc(BuildRpc.buildVrGetLanguageResponse(BuildRpc.getNextId(), (HmiApiLib.Common.Enums.Language)spnLanguage.SelectedItemPosition, (HmiApiLib.Common.Enums.Result)spnResultCode.SelectedItemPosition));
			});
			...
			rpcAlertDialog.SetNegativeButton("Tx Later", (senderAlert, args) =>
			   {
				   ...
				   AppInstanceManager.Instance.sendRpc(BuildRpc.buildVrDeleteCommandResponse(BuildRpc.getNextId(), (HmiApiLib.Common.Enums.Result)spnGeneric.SelectedItemPosition));
			...
			rpcAlertDialog.SetNegativeButton("Tx Later", (senderAlert, args) =>
			   {
				   ...
				   AppInstanceManager.Instance.sendRpc(BuildRpc.buildVrChangeRegistrationResponse(BuildRpc.getNextId(), (HmiApiLib.Common.Enums.Result)spnGeneric.SelectedItemPosition));
			...
			rpcAlertDialog.SetNegativeButton("Tx Later", (senderAlert, args) =>
			   {
				   ...
				   AppInstanceManager.Instance.sendRpc(BuildRpc.buildVrAddCommandResponse(BuildRpc.getNextId(), (HmiApiLib.Common.Enums.Result)spnGeneric.SelectedItemPosition));
		...
		private void CreateUIResponseAlert()
		{
			...
			TextView textViewApplicationId = (TextView)rpcView.FindViewById(Resource.Id.alert_try_again_time_tv);
			EditText editTextdApplicationId = (EditText)rpcView.FindViewById(Resource.Id.alert_try_again_time_et);

			TextView textViewAppExitReason = (TextView)rpcView.FindViewById(Resource.Id.alert_resultcode_tv);
			Spinner spnAppExitReason = (Spinner)rpcView.FindViewById(Resource.Id.alert_resultcode_spn);
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View getSupportedLanguagesRpcView = (View)layoutInflater.Inflate(Resource.Layout.get_support_languages, null);
			...
			languagesButton.Click += delegate
			 {
				 ...
				 View languagesView = (View)layoutInflater.Inflate(Resource.Layout.genericspinner, null);
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.set_display_layout, null);
			rpcAlertDialog.SetView(rpcView);
			...
			addDisplayCapabilitiesButton.Click += delegate
		 {
			 ...
			 View displayCapabilitiesView = (View)layoutInflater.Inflate(Resource.Layout.display_capabilities, null);
			 ...
			 textFieldsButton.Click += delegate
				 {
					 ...
					 View textFieldsView = (View)layoutInflater.Inflate(Resource.Layout.text_field, null);
			 ...
			 imageFieldsButton.Click += delegate
				  {
					  ...
					  View imageFieldsView = (View)layoutInflater.Inflate(Resource.Layout.image_field, null);
			 ...
			 screenParamsButton.Click += delegate
				 {
					 ...
					 View screenParamsView = (View)layoutInflater.Inflate(Resource.Layout.screen_param, null);
		...
		private void CreateUIResponseShow()
		{
			...
			tmpObj = (HmiApiLib.Controllers.UI.OutgoingResponses.Show)AppUtils.getSavedPreferenceValueForRpc<HmiApiLib.Controllers.UI.OutgoingResponses.Show>(adapter.Context, tmpObj.getMethod());
			if (tmpObj != null)
			{
				spnGeneric.SetSelection((int)tmpObj.getResultCode());
			}
		...
		private void CreateUIResponseShowCustomForm()
		{
			...
			TextView textViewInfo = (TextView)rpcView.FindViewById(Resource.Id.alert_try_again_time_tv);
			textViewInfo.Text = "Info";
			EditText editTextdInfo = (EditText)rpcView.FindViewById(Resource.Id.alert_try_again_time_et);
			editTextdInfo.InputType = Android.Text.InputTypes.ClassText;

			TextView textViewResultCode = (TextView)rpcView.FindViewById(Resource.Id.alert_resultcode_tv);
			textViewResultCode.Text = "ResultCode";

			Spinner spnResultCode = (Spinner)rpcView.FindViewById(Resource.Id.alert_resultcode_spn);
			var adapter = new ArrayAdapter<String>(this.Context, Android.Resource.Layout.SimpleSpinnerDropDownItem, resultCode);
			spnResultCode.Adapter = adapter;
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.slider_response, null);
			rpcAlertDialog.SetView(rpcView);
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.tts_get_capabilities, null);
			rpcAlertDialog.SetView(rpcView);
			rpcAlertDialog.SetTitle("GetCapabilities");
			...
			speechCapabilitiesButton.Click += delegate
			{
				...
				View speechCapabilitiesView = (View)layoutInflater.Inflate(Resource.Layout.genericspinner, null);
			...
			prerecordedSpeechButton.Click += delegate
			{
				...
				View prerecordedSpeechView = (View)layoutInflater.Inflate(Resource.Layout.genericspinner, null);
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View getSystemInfoRpcView = (View)layoutInflater.Inflate(Resource.Layout.get_support_languages, null);
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.on_app_activated, null);
			rpcAlertDialog.SetView(rpcView);
			...
			manualAppIdRadioButton.Checked = true;
			...
			registerdAppIdRadioButton.Checked = false;
			...
			appIDCheckBox.CheckedChange += (s, e) =>
			{
				...
				else
				{
					registerdAppIdSpn.Enabled = false;
					manualAppIdEditText.Enabled = false;
					manualRegisterdAppIdRadioGroup.Enabled = false;
					manualAppIdRadioButton.Enabled = false;
					registerdAppIdRadioButton.Enabled = false;
				}
			};
			...
			rpcAlertDialog.SetNegativeButton("Tx Now", (senderAlert, args) =>
			 {
				 ...
				 if (appIDCheckBox.Checked)
				 {
					 if (name.Equals("Registered Apps"))
						 ...
						 if (registerdAppIdSpn.Adapter.Count == 0)
						 {
							 ...
							 return;
                                ...
                                selectedAppID = appIdList[registerdAppIdSpn.SelectedItemPosition];
							 //selectedAppID = Java.Lang.Integer.ParseInt(registerdAppIdSpn.SelectedItem.ToString());
					 ...
					 //:::::::::::::::::::::BuildRpc.buildSdlActivateAppRequest Returning String bt we need RPC Respone
					 //AppInstanceManager.Instance.sendRpc(BuildRpc.buildSdlActivateAppRequest(BuildRpc.getNextId(), selectedAppID));
				 }
			...
			rpcAlertDialog.SetNegativeButton("Tx Now", (senderAlert, args) =>
			 {
				 ...
				 if (appIDCheckBox.Checked)
				 {
					 ...
					 //AppInstanceManager.Instance.sendRpc(BuildRpc.buildSdlGetListOfPermissions(BuildRpc.getNextId(), Java.Lang.Integer.ParseInt(selectedAppID)));
				 }
		...
		private void CreateSDLRequestGetURLS()
		{
			...
			TextView textView = (TextView)rpcView.FindViewById(Resource.Id.policy_file_tv);
			textView.Text = "Service";

			EditText editTextService = (EditText)rpcView.FindViewById(Resource.Id.policy_file_et);
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.alert_ui_response, null);
			rpcAlertDialog.SetView(rpcView);

			TextView textViewMessageCode = (TextView)rpcView.FindViewById(Resource.Id.alert_try_again_time_tv);
			textViewMessageCode.Text = "MessageCode";
			EditText editTextdApplicationId = (EditText)rpcView.FindViewById(Resource.Id.alert_try_again_time_et);
			editTextdApplicationId.InputType = Android.Text.InputTypes.ClassText;

			TextView textViewlanguage = (TextView)rpcView.FindViewById(Resource.Id.alert_resultcode_tv);
			textViewlanguage.Text = "Language";
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.on_allow_sdl_functionality, null);
			rpcAlertDialog.SetView(rpcView);
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.on_app_permission_consent, null);
			rpcAlertDialog.SetView(rpcView);
			...
			consentFunctionsButton.Click += delegate
			{
				...
				View consentFunctionsView = (View)layoutInflater.Inflate(Resource.Layout.permissison_item, null);
		...
		private void CreateSDLNotificationOnReceivedPolicyUpdate()
		{
			...
			TextView policyFileTextView = (TextView)rpcView.FindViewById(Resource.Id.policy_file_tv);
		...
		private void CreateNavigationResponseGetWayPoints()
		{
			...
			CheckBox appIDCheckBox = (CheckBox)getSystemInfoRpcView.FindViewById(Resource.Id.get_way_points_app_id_checkbox);
			appIDCheckBox.Checked = true;


			EditText manualAppIdEditText = (EditText)getSystemInfoRpcView.FindViewById(Resource.Id.get_way_points_app_id_et);
			Spinner registerdAppIdSpn = (Spinner)getSystemInfoRpcView.FindViewById(Resource.Id.get_way_points_app_id_spinner);
			registerdAppIdSpn.Enabled = false;


			RadioGroup manualRegisterdAppIdRadioGroup = getSystemInfoRpcView.FindViewById<RadioGroup>(Resource.Id.get_way_points_app_id_manual_radioGroup);

			RadioButton manualAppIdRadioButton = getSystemInfoRpcView.FindViewById<RadioButton>(Resource.Id.get_way_points_app_id_manual);
			manualAppIdRadioButton.Checked = true;

			RadioButton registerdAppIdRadioButton = getSystemInfoRpcView.FindViewById<RadioButton>(Resource.Id.get_way_points_app_id_registered_apps);
			registerdAppIdRadioButton.Checked = false;
			...
			addLocationDetailsButton.Click += delegate
			{
				...
				View locationDetailsView = (View)layoutInflater.Inflate(Resource.Layout.location_details, null);
			...
			{
			RadioButton manualRegisterdSelectedOptionRadioButton = getSystemInfoRpcView.FindViewById<RadioButton>(manualRegisterdAppIdRadioGroup.CheckedRadioButtonId);
			string name = manualRegisterdSelectedOptionRadioButton.Text;
			...
			int selectedAppID = 0;
                if (appIDCheckBox.Checked)
                {
                    ...
                    AppInstanceManager.Instance.sendRpc(BuildRpc.buildNavGetWayPointsResponse(BuildRpc.getNextId(), (HmiApiLib.Common.Enums.Result)spnResultCode.SelectedItemPosition, selectedAppID, wayPoints));
                }
			});
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.button_get_capabilities_response, null);
			rpcAlertDialog.SetView(rpcView);
			rpcAlertDialog.SetTitle("GetCapabilities");
			...
			rpcAlertDialog.SetNegativeButton("Tx Later", (senderAlert, args) =>
			{
				//::::::::::::::::::BuildRpc.buildButtonsGetCapabilitiesResponse is returning String but we need return type RPCResponse:::::::::::::::::::::
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View getSystemRequestRpcView = (View)layoutInflater.Inflate(Resource.Layout.on_system_request, null);

            rpcAlertDialog.SetView(getSystemRequestRpcView);
			rpcAlertDialog.SetTitle("OnSystemRequest");

			TextView textViewRequestType = (TextView)getSystemRequestRpcView.FindViewById(Resource.Id.request_type_tv);
			Spinner spnRequestType = (Spinner)getSystemRequestRpcView.FindViewById(Resource.Id.request_type_spn);

			TextView textViewURL = (TextView)getSystemRequestRpcView.FindViewById(Resource.Id.url_tv);
			EditText editTextViewURL = (EditText)getSystemRequestRpcView.FindViewById(Resource.Id.url_et);

			TextView textViewFileType = (TextView)getSystemRequestRpcView.FindViewById(Resource.Id.file_type_tv);
			Spinner spnFileType = (Spinner)getSystemRequestRpcView.FindViewById(Resource.Id.file_type_spn);

			TextView textViewOffset = (TextView)getSystemRequestRpcView.FindViewById(Resource.Id.offset_tv);
			EditText editTextOffset = (EditText)getSystemRequestRpcView.FindViewById(Resource.Id.offset_et);

			TextView textViewLength = (TextView)getSystemRequestRpcView.FindViewById(Resource.Id.length_tv);
			EditText editTextLength = (EditText)getSystemRequestRpcView.FindViewById(Resource.Id.length_et);

			TextView textViewTimeOut = (TextView)getSystemRequestRpcView.FindViewById(Resource.Id.timeout_tv);
			EditText editTextTimeOut = (EditText)getSystemRequestRpcView.FindViewById(Resource.Id.time_out_et);

			TextView textViewFileName = (TextView)getSystemRequestRpcView.FindViewById(Resource.Id.file_name_tv);
			...
			rpcAlertDialog.SetNegativeButton("Tx Now", (senderAlert, args) =>
			 ...
			 int selectedAppID = 0;
                 if (appIDCheckBox.Checked)
                 {
                     ...
                     AppInstanceManager.Instance.sendRpc(BuildRpc.buildBasicCommunicationOnSystemRequestNotification((HmiApiLib.Common.Enums.RequestType)spnRequestType.SelectedItemPosition, editTextViewURL.Text, (HmiApiLib.Common.Enums.FileType)spnFileType.SelectedItemPosition,
                                                                                                                     Java.Lang.Integer.ParseInt(editTextOffset.Text), Java.Lang.Integer.ParseInt(editTextLength.Text), Java.Lang.Integer.ParseInt(editTextTimeOut.Text),
                                                                                                                     editTextFileName.Text, selectedAppID));
                 }
			 });
		...
		private void CreateBCNotificationOnReady()
		{
			...
			rpcAlertDialog.SetNegativeButton("Tx Now", (senderAlert, args) =>
			{
                ...
                //AppInstanceManager.Instance.sendRpc(BuildRpc.buildBasicCommunicationOnReady());
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.on_deactivate_HMI, null);
			rpcAlertDialog.SetView(rpcView);
			...
			rpcAlertDialog.SetNegativeButton("Tx Now", (senderAlert, args) =>
			 {
                ...
                AppInstanceManager.Instance.sendRpc(BuildRpc.buildBasicCommunicationOnPhoneCallNotification(checkBoxIsDeactivated.Checked));
			 });
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.on_device_chosen, null);
			rpcAlertDialog.SetView(rpcView);
			rpcAlertDialog.SetTitle("OnFindApplications");
			...
			rpcAlertDialog.SetNegativeButton("Tx Now", (senderAlert, args) =>
			 {
				 AppInstanceManager.Instance.sendRpc(BuildRpc.buildBasicCommunicationOnFindApplicationsNotification(devInfo));
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.on_exit_application, null);
			rpcAlertDialog.SetView(rpcView);
			...
			TextView textViewAppExitReason = (TextView)rpcView.FindViewById(Resource.Id.bc_on_exit_application_app_exit_reason_tv);
			...
			int selectedAppID = 0;
                if (appIDCheckBox.Checked)
                {
                    ...
                    //:::::::::::::::::::::::::BuildRpc.buildBasicCommunicationOnExitAllApplications  method returning String not RPC Message::::::::::::::::::::::::::::::::

                    //AppInstanceManager.Instance.sendRpc(BuildRpc.buildBasicCommunicationOnExitApplication((HmiApiLib.Common.Enums.ApplicationExitReason)spnAppExitReason.SelectedItemPosition, selectedAppID));
		...
		private void CreateBCNotificationOnExitAllApplications()
		{
			...
			rpcAlertDialog.SetNegativeButton("Tx Now", (senderAlert, args) =>
			{
				...
				//AppInstanceManager.Instance.sendRpc(BuildRpc.buildBasicCommunicationOnExitAllApplications((HmiApiLib.Common.Enums.ApplicationsCloseReason)spnGeneric.SelectedItemPosition));
			});
			rpcAlertDialog.SetPositiveButton("Reset", (senderAlert, args) =>
			 {
		...
		private void CreateBCNotificationOnEventChanged()
		{
			...
			var adapter = new ArrayAdapter<String>(this.Context, Android.Resource.Layout.SimpleSpinnerDropDownItem, eventTypes);
		...
		private void CreateBCNotificationOnEmergencyEvent()
		{
			...
			rpcAlertDialog.SetNegativeButton("Tx Now", (senderAlert, args) =>
			 {
                AppInstanceManager.Instance.sendRpc(BuildRpc.buildBasicCommunicationOnEmergencyEventNotification(checkBoxIsDeactivated.Checked));
		...
		private void CreateBCNotificationOnDeviceChosen()
		{
			...
			rpcAlertDialog.SetNegativeButton("Tx Now", (senderAlert, args) =>
			 {
                AppInstanceManager.Instance.sendRpc(BuildRpc.buildBasicCommunicationOnDeviceChosenNotification(devInfo));
		...
		private void CreateBCNotificationOnDeactivateHMI()
		{
			...
			rpcAlertDialog.SetNegativeButton("Tx Now", (senderAlert, args) =>
			 {
                AppInstanceManager.Instance.sendRpc(BuildRpc.buildBasicCommunicationOnDeactivateHMINotification(checkBoxIsDeactivated.Checked));
		...
		private void CreateBCNotificationOnAppDeactivated()
		{
			...
			HmiApiLib.Controllers.BasicCommunication.OutGoingNotifications.OnAppDeactivated tmpObj = new HmiApiLib.Controllers.BasicCommunication.OutGoingNotifications.OnAppDeactivated(); tmpObj = (HmiApiLib.Controllers.BasicCommunication.OutGoingNotifications.OnAppDeactivated)AppUtils.getSavedPreferenceValueForRpc<HmiApiLib.Controllers.BasicCommunication.OutGoingNotifications.OnAppDeactivated>(this.Context, tmpObj.getMethod());
			if (tmpObj != null)
			{
			...
			rpcAlertDialog.SetNegativeButton("Tx Now", (senderAlert, args) =>
			 {
				 ...
				 if (appIDCheckBox.Checked)
				 {
					 ...
					 AppUtils.savePreferenceValueForRpc(this.Context, ((RequestNotifyMessage)rpcMessage).getMethod(), rpcMessage);
					 AppInstanceManager.Instance.sendRpc(rpcMessage);
			...
			rpcAlertDialog.SetPositiveButton("Reset", (senderAlert, args) =>
			 {
				 AppUtils.removeSavedPreferenceValueForRpc(this.Context, tmpObj.getMethod());
			 });
		...
		private void CreateBCNotificationOnAppActivated()
		{
			...
			tmpObj = (HmiApiLib.Controllers.BasicCommunication.OutGoingNotifications.OnAppActivated)AppUtils.getSavedPreferenceValueForRpc<HmiApiLib.Controllers.BasicCommunication.OutGoingNotifications.OnAppActivated>(this.Context, tmpObj.getMethod());
			if (tmpObj != null)
			{
				manualAppIdEditText.Text = tmpObj.getAppId().ToString();
			}
		...
		private void CreateVRResponseGetSupportedLanguages()
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = layoutInflater.Inflate(Resource.Layout.on_touch_event_notification, null);
			rpcAlertDialog.SetView(rpcView);
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.on_reset_timeout, null);
			rpcAlertDialog.SetView(rpcView);
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.subscribe_vehicle_data_response, null);
			rpcAlertDialog.SetView(rpcView);
		...
		private void CreateVIResponseReadDID()
		{
			...
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.read_did_response, null);
			rpcAlertDialog.SetView(rpcView);
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.get_vehicle_type, null);
			rpcAlertDialog.SetView(rpcView);
		...
		{
			AlertDialog.Builder rpcAlertDialog = new AlertDialog.Builder(this.Context);
			View rpcView = (View)layoutInflater.Inflate(Resource.Layout.get_dtc_response, null);
			rpcAlertDialog.SetView(rpcView);
		...
		{
			AlertDialog.Builder getSystemInfoRpcAlertDialog = new AlertDialog.Builder(this.Context);
			View getSystemInfoRpcView = (View)layoutInflater.Inflate(Resource.Layout.get_system_info, null);
			getSystemInfoRpcAlertDialog.SetView(getSystemInfoRpcView);
			...
			rpcAlertDialog.SetNegativeButton("Tx Later", (senderAlert, args) =>
			   {
				   ...
				   AppInstanceManager.Instance.sendRpc(BuildRpc.buildUiSetGlobalPropertiesResponse(BuildRpc.getNextId(), (HmiApiLib.Common.Enums.Result)spnGeneric.SelectedItemPosition));
